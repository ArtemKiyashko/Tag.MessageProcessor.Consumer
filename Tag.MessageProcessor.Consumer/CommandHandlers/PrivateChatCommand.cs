...
public class PrivateChatCommand(ITelegramBotClient telegramBotClient) : ICommandHandler
{
    ...
    public Task ProcessCommand(TgData tgData) 
        => _telegramBotClient.SendTextMessageAsync(tgData.ChatTgId, "Добавть меня в групповой чат и выдай разрешение на упраление чатом, чтобы я мог менять аватарки :)");
}
